...
export default function FlutterInstructorPage() {
  const { t } = useTranslation('flutter')
  const [isZoomOpen, setIsZoomOpen] = useState(false)
  const links = t('instructorPage.links', { returnObjects: true }) as LinkItem[]
  const details = t('instructorPage.details', { returnObjects: true }) as DetailItem[]
        ...
        <div className="container">
          <div className={styles.instructorLayout}>
            ...
            <div className={styles.instructorBio}>
              ...
              <h1 className={styles.title}>{t('instructorPage.heading')}</h1>
              <p className={styles.subtitle}>{t('instructorPage.heroSubtitle')}</p>
              <div className={styles.linkChips}>
                <Link href={t('instructorPage.bioLink')} target="_blank" rel="noopener noreferrer" className={styles.chip}>
                  {t('instructorPage.subheading')}
                </Link>
                {links.map((link) => (
      ...
      <section className={styles.instructorSection}>
        <div className="container">
          <div className={styles.sectionHeader}>
            <h2 className={styles.title}>{t('instructorPage.heroTitle')}</h2>
          </div>
