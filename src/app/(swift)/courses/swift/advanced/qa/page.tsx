...
export default function SwiftQaPage() {
  const { t } = useTranslation('swift')
  const heading = t('qaPage.heading')
  const desc = t('qaPage.description')
  const cta = t('qaPage.cta')
  const url = t('qaPage.url')
  const afterTitle = t('qaPage.afterLearn.title')
  const afterItemsRaw = t('qaPage.afterLearn.items', { returnObjects: true })
        ...
        <div className="container">
          ...
          <div className={styles.afterLearnGrid}>
            {afterItems.map((item, idx) => (
              <div key={item.title} className={styles.afterLearnCard} style={{ ['--after-idx' as string]: idx }}>
