...
describe('buildMetadata', () => {
  it('產生含 OG/Twitter 與 canonical 的完整 metadata', () => {
    ...
    expect(metadata.openGraph?.url).toBe('https://neverland.swift.moe/neverland/sample')
    expect(metadata.openGraph?.images?.[0]).toEqual(
      expect.objectContaining({
        url: 'https://neverland.swift.moe/neverland/og-cover.png',
        width: 1200,
        height: 630,
      })
    )
    expect(metadata.twitter?.card).toBe('summary_large_image')
    expect(metadata.alternates?.canonical).toBe('https://neverland.swift.moe/neverland/sample')
  })
  ...
  it('使用預設值時回傳站點資訊', () => {
    ...
    expect(metadata.openGraph?.url).toBe('https://neverland.swift.moe/neverland/')
  })

  it('可覆寫描述與路徑為根目錄', () => {
    const metadata = buildMetadata({ description: '自訂描述', path: '/' })
    expect(metadata.description).toBe('自訂描述')
    expect(metadata.alternates?.canonical).toBe('https://neverland.swift.moe/neverland/')
  })

  it('多語言 alternate 至少包含 zh-TW', () => {
    const metadata = buildMetadata({ path: '/videos' })
    expect(metadata.alternates?.languages?.['zh-TW']).toBe('https://neverland.swift.moe/neverland/videos')
  })
